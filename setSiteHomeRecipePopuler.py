...
class setSiteHomeRecipePopuler(Base.BaseSetRecipeAd):
    def test_setSiteHomeRecipePopuler(self):
        driver = self.driver
        base_url = self.base_url

        url=base_url+'/admin/v2/data/modules/ads/indexad.php?do=edit&id=42'
        driver.get(url)

        # --------------在这里开始填写要添加的数据，默认为随机，共6条
        info = {
            'name':u'首页-菜谱正流行-from testAutomation',
            'status':u'显示',
            'data':[
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'%s'%(random.choice(data.recipids)),
