...
class setSiteHomeSpecial(Base.BaseSetRecipeAd):
    def test_setSiteHomeSpecial(self):
        driver = self.driver
        base_url = self.base_url

        url=base_url+'/admin/v2/data/modules/ads/indexad.php?do=edit&id=40'
        driver.get(url)

        # --------------在这里开始填写要添加的数据，默认为随机，共6条
        info = {
            'name':u'首页-最新专题-from testAutomation',
            'status':u'显示',
            'data':[
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://www.haodou.com/topic/xueshengshipu',
                    'Limg':random.choice(data.imgLocation),
                },
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://www.haodou.com/topic/miantiao',
                    'Limg':random.choice(data.imgLocation),
                },
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://www.haodou.com/topic/zengfei',
                    'Limg':random.choice(data.imgLocation),
                },
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://www.haodou.com/topic/fangshu',
                    'Limg':random.choice(data.imgLocation),
                },
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://www.haodou.com/topic/baobaoshipu',
                    'Limg':random.choice(data.imgLocation),
                },
            ],
            'startime':'2013-01-01 00:00:00',
            'endtime':'2015-04-01 00:00:00',
            'intro':u'首页-最新专题-from testAutomation',
        }
        ...
        info['startime'] and startime.clear()
        info['startime'] and startime.send_keys(info['startime'])
        info['endtime'] and endtime.clear()
