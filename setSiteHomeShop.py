...
'''
    此部分为添加首页商城广告的方法
    商城广告的特殊之处在于执行之后需要等待缓存更新或者手动清楚缓存
'''

class setSiteHomeShop(Base.BaseSetRecipeAd):
    def test_setSiteHomeShop(self):
        driver = self.driver
        base_url = self.base_url

        url=base_url+'/admin/v2/data/modules/ads/shopad.php?do=edit&id=78'
        driver.get(url)

        # --------------在这里开始填写要添加的数据，默认为随机，共6条
        info = {
            'name':u'首页-商城广告-from testAutomation',
            'status':u'显示',
            'data':[
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://shop.haodou.com/lifeall.php?t=78',
                    'Limg':random.choice(data.imgLocation),
                },
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://shop.haodou.com/lifeall.php?t=5',
                    'Limg':random.choice(data.imgLocation),
                },
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://shop.haodou.com/lifeall.php?t=51',
                    'Limg':random.choice(data.imgLocation),
                },
                {
                    'content':u'%s--from testAutomation'%random.choice(data.remarkdata),
                    'link':'http://shop.haodou.com/lifeall.php?t=8',
                    'Limg':random.choice(data.imgLocation),
                },
            ],
            'startime':'2013-01-01 00:00:00',
            'endtime':'2015-04-01 00:00:00',
            'intro':u'首页-菜谱正流行-from testAutomation',
        }
