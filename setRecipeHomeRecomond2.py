...
'''
    此部分为菜谱首页精彩推荐模块的方法
'''
class setRecipeHomeRecommond2(Base.BaseSetRecipeAd):
    def test_setRecipeHomeRecmmond2(self):
        driver = self.driver
        base_url = self.base_url

        #这里填写要设置哪几个
        url=base_url+'/admin/v2/data/modules/ads/recipead.php?do=edit&id=71'
        driver.get(url)

        # --------------在这里开始填写要添加的数据，默认为随机,所添加的数据条目不能大于5
        info = {
            'name':u'菜谱首页右下角精彩推荐-from testAutomation',
            'status':u'显示',
            'data':[
                {
                    'content':random.choice(data.remarkdata),
